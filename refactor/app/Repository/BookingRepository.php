...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($jobs) {
            $jobs->each(function($job) use (&$emergencyJobs,&$noramlJobs){

                $emergencyJob   =   $job->where('immediate','yes')->sortBy('due');
                $noramlJob      =   $job->where('immediate','!=','yes');

                $emergencyJobs[] = collect($emergencyJob);
                $noramlJobs[]    = collect($noramlJob);

            });
                Job::checkParticularJob($user_id, $noramlJobs);
            }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($customer && $customer->is('customer')) {
            $jobs = $customer->jobs()
        ...
        } elseif ($customer && $customer->is('translator')) {
            
            $jobs = Job::getTranslatorJobsHistoric($customer->id, 'historic', $pagenum);
            $usertype = 'translator';
            ...
            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'jobs' => $jobs, 'customer' => $customer, 'usertype' => $usertype, 'numpages' => $numpages, 'pagenum' => $pagenum];
    }
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        $user_meta = UserMeta::where('user_id', $user_id)
            ->select('user_id','translator_type','gender','translator_level')
            ->first();
        ...
        $filteredJobIds = collect($job_ids)->filter(function ($job) use ($user_id) {
            $job = Job::find($job->id);
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        $delpay_translator_array = array();     // suitable translators (need to delay push)
            ...
            $translator_array = $users->filter(function ($oneUser) use ($data, $job) {
                if (!$this->isNeedToSendPush($oneUser->id)) return false;

                $not_get_emergency = TeHelper::getUsermeta($oneUser->id, 'not_get_emergency');
                if ($data['immediate'] == 'yes' && $not_get_emergency == 'yes') return false;
                ...
                foreach ($jobs as $oneJob) {
                    if ($job->id == $oneJob->id) {
                        ...
                        if ($job_for_translator == 'SpecificJob') {
                            ...
                            return $job_checker != 'userCanNotAcceptJob';
                        }
                    }
                }
            ...
            $delpay_translator_array = $translator_array->filter(function ($oneUser) {
                    return $this->isNeedToDelayPush($oneUser->id);
                });

            $translator_array = $translator_array->reject(function ($oneUser) use ($delpay_translator_array) {
                    return $delpay_translator_array->contains('id', $oneUser->id);
                });
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $dueDateAndTime = Carbon::parse($job->due);
        $date = $dueDateAndTime->format('d.m.Y');
        $time = $dueDateAndTime->format('H:i');
    ...
    private function changeTimedoutStatus($job, $data, $changedTranslator)
    {
        ...
        if ($data['status'] == 'pending') {
            ...
            return true;
        } elseif ($changedTranslator && $job->save()) {
